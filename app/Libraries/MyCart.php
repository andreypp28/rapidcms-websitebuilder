...
class MyCart
{
    ...
    */
    ...
    public static function placeOrder($currency, $transactionId='', $payerId='', $orderStatus=ORDER_PENDING)
    {
        ...
        {
            $order->customer_name   = $billingAddress['first_name'] . ' ' . $billingAddress['last_name'];
            $order->customer_email  = $billingAddress['email'];    
        }
    ...
    public static function addCardItem($productId, $discountId, $quantityId, $options)
    {
        ...
        $basePrice = 0;
        $totalQuantity = 0;
        $optionRows = getCardOptionRows($productId, $options);
        $customSets = array();
        ...
        if(!is_array($quantityId)) //normal set
        {
            ...
            //get base price
            $basePrice = $sets * getCardPrice($productId, $quantityId, $optionRows);
        ...
        else //custom sets
        {
            ...
            foreach($quantityId as $row) 
            {
                ...
                //get base price
                $basePrice += getCardPrice($productId, $row['quantity'], $optionRows);
    ...
    public static function addTemplateItem($productId, $discountId, $quantityId)
    {
        $sets = 1;
        $discountPercent = 0;
        $basePrice = 0;
    ...
    public static function addDesign($productId)
    {
...
* @return []
*/
function getCardOptionRows($productId, $options)
{
    $optionRows = array();
