...
class CartController extends FrontController {
    ...
    public function checkout()
    {
        ...
        if($this->auth->isLoggedIn())
        {
            ...
            $regions = Region::where('country_id', $customer->country_id)
                ->lists('name', 'id');
        }
        else
        {
            ...
            $regions = Region::where('country_id', $firstKey)
                ->lists('name', 'id');
        }
        //end country & region
        ...
        if($cart->isShippingRequired())
            return $this->view('cart.checkout', compact('customer', 'countries', 'regions', 'shipping_methods'));    
        else
            return $this->view('cart.checkout_noshipping', compact('customer', 'countries', 'regions'));    
    }
    ...
    public function addCard()
    {
        if($saved = MyCart::hasSavedCard())
        {
            ...
            if(MyCart::addCardItem($productId, $discountId, $quantityId, $options))
            {
    ...
    public function addTemplate()
    {
        if($saved = MyCart::hasSavedTemplate())
        {
            ...
            $discountId = isset($saved['discount']) ? $saved['discount'] : 0;
            $quantityId = $saved['quantity'];
            if(MyCart::addTemplateItem($productId, $discountId, $quantityId))
            {
                MyCart::removeSavedItem();
    ...
    public function purchase()
    {
        ...
        else if($payment == PAYMENT_CREDIT)
        {
            ...
            $params = array(
                ...
                //'returnUrl' => route('cart.checkout.purchase.paypal'),
    ...
    public function ajaxGetBilling()
    {
        ...
        $regions = Region::where('country_id', $address['country'])
                ->lists('name', 'id');
            
        return $this->view('cart.checkout.billing', compact('address', 'cart', 'countries', 'regions'));    
        exit;
    }
