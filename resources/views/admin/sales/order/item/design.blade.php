<?php $designInfo = $item->designInfo(); ?>
...
<tbody>
    ...
    <tr>
        ...
        @if($item->job_status != JOB_COMPLETED)
        <td class="text-right" style="padding-top:20px;">Stuff</td>
        <td>{!! Form::select('job_manager', $stuffs, $item->job_stuff_id, array('class' => 'form-control', 'required')) !!}</td>
        <td><input type="submit" class="btn btn-info" name="btn-assign-job" value="Submit"/></td>
        @endif
        
        @if($item->job_status == JOB_COMPLETED)
        <td class="text-right">Stuff</td>
        <td colspan="2">
            @if(isset($item->stuff->first_name))
                {{ $item->stuff->first_name }} {{ $item->stuff->last_name }}
            @endif
        </td>
        @endif
    </tr>
    ...
    <tr>
        ...
        <td class="text-middle">{!! $item->status_lang(true) !!}</td>
        <td class="text-middle">
            @if($item->job_status == JOB_WORKING)
